...
export function readWebSourceWhitelist(storage) {
  ...
  try {
    ...
    const savedUrls = new Set(normalizedSaved.map((entry) => entry.url));
    return normalizeWebSourceWhitelist([
      ...normalizedSaved,
      ...DEFAULT_WEB_SOURCE_WHITELIST.filter((entry) => !savedUrls.has(entry.url)),
    ]);
  } catch {
