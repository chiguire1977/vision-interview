...
const BROAD_RESEARCH_TERMS = "(面试 OR 面试题 OR 面试问题 OR 题目 OR 知识点 OR 原理 OR 教程 OR 工程实践 OR 排障)";
...
export function expandResearchQuery(baseQuery) {
  const query = String(baseQuery || "").trim().replace(/\s+/g, " ");
  return query ? query + " " + BROAD_RESEARCH_TERMS : "";
}
...
export async function retrieveWebSources({ query, excludedTitles = [], fetchImpl = globalThis.fetch, storage, now = Date.now(), options = {}, signal } = {}) {
  ...
  const queries = buildResearchQueries(query, normalizedOptions.whitelist);
  ...
  for (let offset = 0; offset < queries.length && sources.length < normalizedOptions.minimumSources; offset += normalizedOptions.parallelRequests) {
    if (signal?.aborted) throw signal.reason ?? new DOMException("联网检索已终止", "AbortError");
    const batch = queries.slice(offset, offset + normalizedOptions.parallelRequests);
    const controllers = [];
    let finished = false;
    ...
    await new Promise((resolve) => {
      ...
      const finish = (reason = "completed") => {
        ...
        if (reason !== "completed") {
          const abortReason = reason === "enough"
            ? new DOMException("已获得足够检索资料", "AbortError")
            : new DOMException("本批联网检索超时", "TimeoutError");
          controllers.forEach((controller) => controller.abort(abortReason));
        }
        resolve(undefined);
      };
      const requests = batch.map(async (researchQuery) => {
        const timeoutController = new AbortController();
        controllers.push(timeoutController);
        const requestTimer = setTimeout(() => timeoutController.abort(new DOMException("联网检索超时", "TimeoutError")), normalizedOptions.requestTimeoutMs);
      ...
      timer = setTimeout(() => finish("timeout"), normalizedOptions.softTimeoutMs);
      requests.forEach((request) => {
        request
          .then((incoming) => {
            if (finished) return;
            sources = mergeSources(sources, incoming, 8, normalizedOptions.whitelist);
            if (sources.length >= normalizedOptions.minimumSources) finish("enough");
          })
          .catch(() => { failureCount += 1; })
          .finally(() => {
            completedCount += 1;
            if (completedCount === requests.length) finish();
          });
      });
    });
    if (signal?.aborted) throw signal.reason ?? new DOMException("联网检索已终止", "AbortError");
    if (sources.length >= normalizedOptions.minimumSources) {
      earlyReturned = true;
      break;
    }
  }
  ...
  return { sources, cached: false, earlyReturned, ...(failureCount === queries.length ? { error: "联网检索暂不可用" } : {}) };
}
