...
function recordWeight(record) {
  const mastery = record.masteryStage === "未掌握" ? "低" : record.masteryStage === "部分掌握" ? "中" : record.mastery;
  const masteryWeight = mastery === "低" ? 7 : mastery === "中" ? 2 : 0;
...
export function buildLearningFocus(records) {
  ...
  const validRecords = records.filter((record) => {
    ...
    if (record.skipReason === "不感兴趣" || record.skipReason === "稍后再学") return false;
    return record.mastery === "低" || record.mastery === "中" || record.masteryStage === "未掌握" || record.masteryStage === "部分掌握" || record.action === "跳过题目";
  });
  ...
  for (const record of validRecords) {
    ...
    current.attempts += 1;
    if (record.mastery === "低" || record.masteryStage === "未掌握") current.lowCount += 1;
    if (record.mastery === "中" || record.masteryStage === "部分掌握") current.mediumCount += 1;
  ...
  const focusRecords = validRecords.filter((record) => record.mastery === "低" || record.masteryStage === "未掌握" || record.action === "跳过题目" || record.reviewIssues?.length);
...
export function recommendDifficultyAdjustment(records) {
  if (!Array.isArray(records)) return null;
  const recent = [...records]
    .filter((record) => record && typeof record === "object")
    .sort((left, right) => Date.parse(right.timestamp || right.date || 0) - Date.parse(left.timestamp || left.date || 0));
  let consecutiveTooHard = 0;
  for (const record of recent) {
    if (record.action !== "跳过题目" || record.skipReason !== "太难") break;
    consecutiveTooHard += 1;
  }
  return consecutiveTooHard >= 3 ? "降低难度" : null;
}
