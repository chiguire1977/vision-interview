...
export function filterRuntimeLogs(value, level = "ALL") {
  const logs = normalizeRuntimeLogs(value);
  return level === "ALL" ? logs : logs.filter((log) => log.level === level);
}
...
export function createCloseBackupPayload(previousSnapshot, currentSnapshot, maxBytes = CLOSE_PAYLOAD_LIMIT) {
  ...
  for (const key of BACKUP_KEYS) {
    ...
    data[key] = compacted;
    if (key === "vision-interview-projects") replaceKeys.push(key);
  }
...
export async function saveBackupToGitHub(options) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    let data = options.merge
      ? mergeBackupData(options.data, remoteArchive?.data, { preferLocal: true })
