...
export { WEB_SOURCE_WHITELIST_STORAGE_KEY };
...
export function sanitizeBackupData(value) {
  ...
  for (const key of BACKUP_KEYS) {
    ...
    if (key === WEB_SOURCE_WHITELIST_STORAGE_KEY) {
      result[key] = normalizeWebSourceWhitelist(value[key]);
      continue;
    }
...
function isEmptyBackupValue(value) {
...
export async function saveBackupToGitHub(options) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    const remoteData = sanitizeBackupData(remoteArchive?.data);
    const collapsedKeys = (Array.isArray(options.replaceKeys) ? options.replaceKeys : [])
      .filter((key) => COLLECTION_KEYS.has(key)
        && Object.prototype.hasOwnProperty.call(remoteData, key)
        && !isEmptyBackupValue(remoteData[key])
        && isEmptyBackupValue(data[key]));
    const archiveCollapsed = Object.keys(remoteData).length > 0 && Object.keys(data).length === 0;
    if (archiveCollapsed || collapsedKeys.length > 0) {
      const backupPath = `${path}.bak`;
      const previousBackup = await readGitHubFile({ fetchImpl, token, owner, repo, branch, path: backupPath });
      const backupResponse = await fetchImpl(contentsUrl({ owner, repo, branch, path: backupPath }), {
        method: "PUT",
        headers: githubHeaders(token, true),
        body: JSON.stringify({
          message: `chore(data): preserve backup before collapse ${now}`,
          content: current?.content || encodeBase64Utf8(`${JSON.stringify(remoteArchive, null, 2)}\n`),
          branch,
          ...(previousBackup?.sha ? { sha: previousBackup.sha } : {}),
        }),
      });
      if (!backupResponse.ok) {
        const detail = await backupResponse.text();
        throw new Error(`已阻止空备份覆盖非空数据，但 .bak 写入失败（HTTP ${backupResponse.status}）：${detail.slice(0, 240)}`);
      }
      const scope = collapsedKeys.length ? `（${collapsedKeys.join("、")}）` : "";
      throw new Error(`已阻止空备份覆盖非空数据${scope}；原备份已保存为 .bak，请检查本地数据加载状态。`);
    }
